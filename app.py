...
st.markdown("# Genomisches Neugeborenenscreening")  # Kleiner Titel
...
if st.session_state.df is None:
    ...
    if uploaded_file is not None:
        with st.spinner('Lade...'):
            ...
            # Vollständige Namen extrahieren
            gene_dict = {}
            for col in df.columns:
                if 'Gen: ' in col and 'Erkrankung: ' in col and 'nationalen' in col and '[Kommentar]' not in col:
                    parts = col.split('Gen: ')
                    if len(parts) > 1:
                        gene_part = parts[1].split('  Erkrankung: ')
                        if len(gene_part) > 1:
                            gene = gene_part[0].strip()
                            disease = gene_part[1].split('  ')[0].split('"')[0].strip()  # Vollständig bis " oder Space
    ...
    for tab_idx, tab in enumerate(tabs):
        with tab:
            ...
            with left_col:
                ...
                fig_nat = go.Figure()
                fig_nat.add_trace(go.Bar(name='Ja', x=[''], y=[ja_pct], marker_color='#A7F3D0', 
                                         text=f'{ja_pct:.0f}%', textposition='inside', textfont_size=12))
                fig_nat.add_trace(go.Bar(name='Nein', x=[''], y=[nein_pct], marker_color='#FEE2E2', 
                                         text=f'{nein_pct:.0f}%', textposition='inside', textfont_size=12))
                fig_nat.update_layout(barmode='stack', height=220, margin=dict(b=0,t=0,l=0,r=0), 
                                      yaxis_range=[0,100], showlegend=False, bargap=0.1)
                st.plotly_chart(fig_nat, use_container_width=True, key=f'nat_stack_{gene}_{tab_idx}')
            ...
            with right_col:
                ...
                nein_pct_stud = 100 - ja_pct_stud
                
                fig_stud = go.Figure()
                fig_stud.add_trace(go.Bar(name='Ja', x=[''], y=[ja_pct_stud], marker_color='#BAE6FD', 
                                          text=f'{ja_pct_stud:.0f}%', textposition='inside', textfont_size=12))
                fig_stud.add_trace(go.Bar(name='Nein', x=[''], y=[nein_pct_stud], marker_color='#F0F9FF', 
                                          text=f'{nein_pct_stud:.0f}%', textposition='inside', textfont_size=12))
                fig_stud.update_layout(barmode='stack', height=220, margin=dict(b=0,t=0,l=0,r=0), 
                                       yaxis_range=[0,100], showlegend=False, bargap=0.1)
                st.plotly_chart(fig_stud, use_container_width=True, key=f'stud_stack_{gene}_{tab_idx}')
